package com.example.basecase.controller;


import com.example.basecase.VO.DeleListRestfulVO;
import com.example.basecase.VO.ReleaseListRestfulVO;
import com.example.basecase.utils.DateUtils;
import com.example.basecase.VO.ResponseVO;
import com.example.basecase.webservice.documentservice.*;
...
public class DocumentController {
    ...
    @Resource
    DocumentPortType documentPort;


    @PostMapping("/addFile")
    public ResponseVO build(@RequestBody DocVO docVO) {

        Doc doc = documentPort.buildOneDoc(docVO);
    ...
    public ResponseVO delete(@RequestBody DeleListRestfulVO del) {
        ...
        for(int i = 0; i < delList.size(); i++) {
            ...
            if (documentPort.findDocById(docId) != null) {
                documentPort.deleteById(docId);
    ...
    public ResponseVO Import(@RequestParam("file") MultipartFile file) throws IOException, IOException_Exception {
        ...
        Doc doc = documentPort._import(list);
        if (doc != null){
    ...
    public ResponseVO modify(@RequestBody DocVO doc) {
        ...
        if ( documentPort.findDocById(doc.getDocId()) == null) {
            System.out.println("------------------文书不存在------------------");
            return ResponseVO.buildFailure("文书不存在");
        }

        Doc docPO = documentPort.updateOneDoc(doc);
    ...
    public ResponseVO release(@RequestBody ReleaseListRestfulVO releaseListVO) {
        ...
        for (int i = 0; i < length; i++) {
            Doc temp = documentPort.findDocById(Long.parseLong(releaseList.get(i)));
            if (temp == null) {
                flag = true;
                break;
            }
                ...
                if(length > 1){
                    temp.setState(1);
                    temp.setPublishDate(DateUtils.dateToXmlDate(new Date(System.currentTimeMillis())));
        ...
        if (flag) {
            ...
            documentPort.releaseDocs(releaseDocList);
    ...
    public ResponseVO query(@RequestBody QueryBody body){
        ...
        return ResponseVO.buildSuccess(documentPort.queryDocs(body));
    }

    @GetMapping("/getFileById/{id}")
    public ResponseVO getFileById(@PathVariable("id")String id){
        System.out.println("测试id"+id);
        return ResponseVO.buildSuccess(documentPort.findDocById(Long.parseLong(id)));
    }
